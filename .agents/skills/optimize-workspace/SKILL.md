...
description: Analyze and optimize another Codex workspace's configuration
disable-model-invocation: true
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, Task, WebFetch, AskUserQuestion
argument-hint: "[path-to-workspace]"
---

Optimize the Codex workspace at `$ARGUMENTS`.
...
5. **Check existing Codex config** -- AGENTS.md, .Codex/, .mcp.json, hooks, other AI configs.
...
7.5. **Coordination protocol detection** -- Scan for multi-thread signals:
   ...
   - Multiple distinct workstreams in flight (scan AGENTS.md, README, recent commits)
   ...
   - Add Coordination section to AGENTS.md pointing at `~/.Codex/rules/coordination.md`
...
8. **Fix permissions** (Phase 2) -- Create `.Codex/settings.json` with `$schema` reference, stack-specific allows and deny rules. Use explicit git subcommands, not `Bash(git *)`. Use `//path` for absolute paths (not `/path`). Consider `dontAsk` mode for locked-down workflows.

9. **Optimize AGENTS.md** (Phase 3) -- Under 500 lines. Add gotchas, business context. Apply selected patterns (current-state-capsule, collaboration-posture, blocked-task-tracking, etc.). Use `@path` imports.

10. **Add rules** (Phase 4) -- Always deploy:
    - `.Codex/rules/context-capsule.toml` (copy from optimizer workspace)
    - `.Codex/rules/windows-shell.md` (copy from optimizer workspace, for Windows targets)
...
14. **Public repo lock** (Phase 7.5) -- Check if any remote is public:
    ...
    - If yes: create `.public-repo` marker, install pre-commit hook (blocks `git commit` unless `PUBLIC_REPO_VERIFIED=1`), install commit-msg hook (strips Co-Authored-By). Add `scripts/verified-commit.sh` wrapper. Add `Bash(scripts/verified-commit.sh *)` to `.Codex/settings.json` allow list. Document the workflow in AGENTS.md.
...
17. **Commit** -- `git add -A && git commit -m "Codex optimization"`. User can `git diff HEAD~1` to review or `git revert HEAD` to undo.

18. **Report** -- Summarize all changes and rationale. Note anything needing user customization.

19. **Clean up optimizer workspace** -- Check `CC-Optimizer/.Codex/settings.local.json` for stale entries accumulated from "always allow" clicks during this optimization. Remove any that are already covered by project or global settings. Keep only `WebSearch`.

## Key Principles
- Follow the playbook, not raw docs
- LLMs are mortar, scripts are bricks
- Permission friction is the #1 blocker -- fix it first
- AGENTS.md under 500 lines
