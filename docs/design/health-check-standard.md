...
node names, SSH credentials) belong in `health-check-environments.md` (gitignored).
...
The skill uses `!` backtick preprocessing to inject live data:
...
Environment-specific details are in `health-check-environments.md` (gitignored).
The patterns below describe what each environment TYPE should check, without
revealing specific hostnames, credentials, or service inventories.
...
- **Health script -- internal (runs on server over SSH):**
  ...
  - Must be compatible with the server's Python version (may be old --
    no f-strings, no walrus operator, no dataclasses, use .format())
