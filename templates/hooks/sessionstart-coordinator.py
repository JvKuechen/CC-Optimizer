...
def main():
    ...
    if is_coordinator:
        ctx = (
            ...
            "1. The handoff capsule (auto-injected from capsule.toml) is the durable spine: role,\n"
            "   current state, holds/gates, next safe action. Trust it as current truth. Wave\n"
            "   history is git log, not a prose file.\n"
            "2. Check worker readiness from git truth (team/worktrees-status.sh if present), not memory.\n"
            "3. Rebuild the in-chat bounty board from git log + the injected capsule.\n"
